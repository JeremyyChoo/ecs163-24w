...
const zoom = d3.zoom()
.scaleExtent([1, 40]) // Limit zoom scale
...
.y(function(d) { return y[d.dimension](d.value); });
...
.enter().append("path")
  .attr("class", "line")
  .attr("d", function(d) {
    return line(dimensions.map(function(p) { return {dimension: p, value: d[p]}; }));
  })
  .style("fill", "none")
  .style("stroke", "#69b3a2")
  .style("opacity", 0.5);

// Function to update chart on zoom/pan
...
svg.selectAll("g.axis").call(d3.axisLeft().scale(newX));
}

// Append the axis as usual
...
function drawPieChart() {
    d3.csv("mxmh_survey_results.csv").then(function(data) {
        ...
        // Create a group element for the pie chart and apply the transform to center it
        const g = svg.append("g")
            .attr("transform", `translate(${margin.left + width / 2}, ${margin.top + height / 2})`);

        // Define the zoom behavior
        const zoom = d3.zoom()
            .scaleExtent([0.1, 10]) // Limits for zoom scale
            .on("zoom", (event) => {
                g.attr("transform", event.transform); // Apply the zoom behavior
            });
